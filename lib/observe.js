...
function createChangesObservable(topic, replayId, saveReplayId, logger) {
  ...
  topic.subscribeWithReplay(replayId, [loggingExt, authFailureExt], data => {
    try {
 ...
 */
AuthFailureExtension = function(logger) {
  this.incoming = function(message, callback) {
    if (
      message.channel === '/meta/connect'
      && typeof(message.error) === "string"
      && message.error.match(/^401/)
    ) {
 ...
 */
LoggingExtension = function(logger) {
  this.incoming = function(message, callback) {
