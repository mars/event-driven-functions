...
function writeToParquet(schemaAndRecordsObservable, env, logger) {
  ...
  schemaAndRecordsObservable
    ...
    .subscribe({
      next: x => {
        try {
          targetSchemas[x.object] = sObjectDescribeToParquetSchema(x.content, x.object, logger)
        } catch (err) {
          logger(`!      writeToParquet failed for "${object}" schema: ${err.stack}`)
          throw data;
        }
      }
    });
      ...
      next: x => {
        try {
          ...
          const writer = targetWriters[object] = targetWriters[object] ||
            new parquet.ParquetWriter(parquetFilepath(object, env.OUTPUT_PATH), parquetSchema, 'snappy');
          writer.write(data);
        } catch (err) {
          logger(`!      writeToParquet failed for "${object}" records: ${err.stack}`)
          throw data;
        }
      },
      complete: () => {
