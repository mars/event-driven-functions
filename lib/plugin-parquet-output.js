    ...
    .subscribe({
      next: x => {
        ...
        const writer = targetWriters[object] = targetWriters[object] ||
          new parquet.ParquetWriter(parquetFilename(object), parquetSchema, 'snappy');
        writer.write(data);
      },
      complete: () => {
        ...
        if (env.BUCKET_NAME != null) {
          const s3Puts = Object.entries(targetWriters).map( v => {
            const filename = parquetFilename(v[0]);
...
function parquetFilename(objectName) {
  return `salesforce-${objectName}.parquet`;
}
