...
function* run(env, logStream) {
  ...
  recordsObservable.subscribe({
    next: x => console.log(`recordsObservable received: ${JSON.stringify(x, null, 2)}`),
    error: err => console.error(`recordsObservable error: ${err}`),
    complete: () => console.log('recordsObservable complete')
  });
}
