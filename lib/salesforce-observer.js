...
function observe(salesforceApi, topicNames, rxSubject, env, logger = loggers.default) {
  ...
  const authFailureExt = new salesforceApi.streaming.AuthFailureExtension(exitCallback);
  const fayeClient = salesforceApi.streaming.withExtensions([ authFailureExt ]);
...
function subscribeAndPush(
  ...
  if (replayId != null) {
    ...
    const replayExt = new salesforceApi.streaming.ReplayExtension(topicName, replayId);
